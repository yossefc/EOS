...
def upgrade():
    ...
    conn.execute(sa.text(
        "UPDATE donnees_enqueteur SET elements_retrouves = LEFT(elements_retrouves, 10) WHERE LENGTH(elements_retrouves) > 10"
    ))
    conn.execute(sa.text(
        "UPDATE enquete_facturation SET tarif_eos_code = LEFT(tarif_eos_code, 10) WHERE LENGTH(tarif_eos_code) > 10"
    ))
    conn.execute(sa.text(
        "UPDATE enquete_facturation SET tarif_enqueteur_code = LEFT(tarif_enqueteur_code, 10) WHERE LENGTH(tarif_enqueteur_code) > 10"
    ))
    ...
    with op.batch_alter_table('donnees', schema=None) as batch_op:
        ...
        batch_op.alter_column('motif',
               ...
               existing_nullable=True)
        batch_op.drop_index('idx_donnee_datenaissance_maj')
        batch_op.create_index(batch_op.f('ix_donnees_client_id'), ['client_id'], unique=False)
    ...
    with op.batch_alter_table('donnees_enqueteur', schema=None) as batch_op:
        batch_op.alter_column('elements_retrouves',
               ...
               type_=sa.String(length=10),
               existing_nullable=True)
        batch_op.create_index(batch_op.f('ix_donnees_enqueteur_client_id'), ['client_id'], unique=False)
        batch_op.drop_constraint('donnees_enqueteur_donnee_id_fkey', type_='foreignkey')
        batch_op.create_foreign_key(None, 'donnees', ['donnee_id'], ['id'])

    with op.batch_alter_table('enquete_archive_files', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_enquete_archive_files_client_id'), ['client_id'], unique=False)
    ...
    with op.batch_alter_table('export_batches', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_export_batches_client_id'), ['client_id'], unique=False)

    with op.batch_alter_table('partner_case_requests', schema=None) as batch_op:
        batch_op.drop_constraint('partner_case_requests_donnee_id_request_code_key', type_='unique')
        batch_op.create_unique_constraint('uq_partner_case_request', ['donnee_id', 'request_code'])

    with op.batch_alter_table('partner_tarif_rules', schema=None) as batch_op:
        batch_op.drop_constraint('partner_tarif_rules_client_id_tarif_lettre_request_key_key', type_='unique')
        batch_op.create_unique_constraint('uq_partner_tarif_rule', ['client_id', 'tarif_lettre', 'request_key'])

    with op.batch_alter_table('tarifs_enqueteur', schema=None) as batch_op:
        batch_op.drop_index('ix_tarifs_enqueteur_client_id')
