"""
Script pour ajouter le mapping NJF au profil PARTNER ENQUETES
"""
import sys
import os
...
def add_njf_mapping():
    """Ajoute le mapping NJF au profil PARTNER ENQUETES"""
    ...
    existing = ImportFieldMapping.query.filter_by(
        ...
        internal_field='nomPatronymique'
    ).first()
    
    if existing:
        logger.info("⏭️  Mapping NJF (nomPatronymique) déjà existant (ignoré)")
        return 0
    ...
    logger.info("✅ Mapping NJF (nomPatronymique) ajouté avec succès")
    return 0
